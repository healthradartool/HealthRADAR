...
plot_df <- costing_df %>%
  pivot_longer(cols = c(TotalDonorReported, TotalCountryReported), names_to = "Type", values_to = "Value")

# Ensure 'Year' is treated as a factor and ordered correctly
plot_df$Year <- as.numeric(plot_df$Year)

# Create the plot
ggplot(plot_df, aes(x = Year, y = Value / 1000, color = Type)) +
  geom_line(size = 1) +
  facet_wrap(~ Country, scales = "free_y") +
  scale_x_continuous(breaks = unique(plot_df$Year)) +
  scale_color_manual(values = c("TotalDonorReported" = "#440154", "TotalCountryReported" = "#1f9e89"),
                     labels = c("TotalDonorReported" = "Reported by donor", "TotalCountryReported" = "Reported by country")) +
  labs(
    title = "Comparison of Total Reported Contributions",
    subtitle = "For Selected Countries (2020-2022)",
    x = "Year",
    y = "Total Contributions (in thousands)",
    color = "Contribution Source",
    caption = "Source: WMR2023 Annex 4C"
  ) +
  theme_minimal() +
  theme(
...
```{r}
# Data for ITN distribution and effectiveness
itn_dist <- c(5000, 2000, 1500, 0, 5000, 4500, 5500, 0, 0)
people_protected_per_net <- 2
itn_eff <- c(0.95, 0.90, 0.85)

# Calculate the effective coverage using convolution
itn_cov <- convolve(itn_dist * people_protected_per_net, rev(itn_eff), type = "open")
itn_cov <- itn_cov[1:length(itn_dist)]  # Match the length of the distribution data
itn_cov_rounded <- round(itn_cov, 2)

# Create a data frame for the plot
years <- seq_along(itn_dist)
itn_data <- data.frame(
  Year = years,
  Effective_Coverage = itn_cov_rounded
)

# Create the ITN coverage plot with integer x-axis increments and y-axis limits
itn_coverage_plot <- ggplot(itn_data, aes(x = Year, y = Effective_Coverage)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  geom_text(aes(label = Effective_Coverage), vjust = -0.5, size = 5, colour = "black") +
  labs(
    title = "Effective ITN Coverage Over Time",
    subtitle = "Malaria prevention through distribution and use of ITNs",
    x = "Year",
    y = "Effective Coverage (People Protected)",
    caption = "Source: Hypothetical example. Assuming 2 people are protected per ITN and that ITN protection wanes over time."
  ) +
  scale_x_continuous(breaks = seq(min(itn_data$Year), max(itn_data$Year), by = 1)) +
...
Modeling malaria transmission at a national level can overlook important spatial heterogeneities:
