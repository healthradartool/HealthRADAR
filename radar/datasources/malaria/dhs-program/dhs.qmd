...
LLIN_access <- access_data |>
  ...
  mutate(time = seq(365, 365*17, by = 365)) |>  
...
-   Net functionality and effectiveness wanes at a rate of 25% per annum, and LLINs are considered "expired" after three years.

-   Operational effectiveness: 65% of actual bites from an indoor-biting infectious mosquito are assumed to be prevented by LLINs used during peak biting times.

### Coverage

Coverage is dependent on access to LLINs at the distribution rate of two persons per net. This can be obtained from data from the World Malaria Report Annex *4-D* detailing the number of ITNs distributed in a given year. Distribution is generally done at a rate of one net per two persons at risk and divided by the total population at risk.

$$
\textrm {Access at time t} = \frac {\textrm{Nets distributed} \times 2}{\textrm {Population at risk}} - \textrm {losses due to insecticide waning and wear and tear}
$$
...
The World Malaria Report only provides values for the three years preceding the report, but we show both values for access below for illustrative purposes:
...
ggplot() +
  geom_line(data = model_output_df, aes(x = year, y = llin_eff_cov, colour = Data)) +
    geom_line(data = model_output_df |>  mutate(Data = "DHS Acccess"), aes(x = year, y = LLIN, colour = Data)) +
  geom_col(data = WHO_data, aes(x = Year, y = Proportion_population, fill = Data)) +
  ...
  labs(title = "Effective LLIN Coverage in Kenya" ,
    ...
    caption = str_wrap("Note the different data sources. DHS' data captures access to LLINs at particular points in time, but this data may be incongruent with annual LLIN access data captured in the World Malaria Report. We also expect effective coverage to be lower when accounting for operational effectiveness, usage and waning in the insecticide efficacy and physical integrity of the net. Sources: DHS Data API - Malaria Indicators ML_NETP_H_IT2 & ML_NETP_H_LL2; World Malaria Report Annexes 4-D and 4-F.")) 
