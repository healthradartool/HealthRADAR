...
itn_access <- access_data |>
  ...
  complete(SurveyYear = full_seq(SurveyYear, 1), fill = list(Value = 0)) |> 
  pull(Value)

itn_use <- usage_data |>
  arrange(SurveyYear) |>
  select(SurveyYear, Value) |> 
  complete(SurveyYear = full_seq(SurveyYear, 1)) |> 
...
Coverage is further considered effective if it meets the minimum access of one ITN per two persons, the net is adequately used (and as a proxy, used the night prior to the survey), and is operationally effective at preventing bites from an infectious mosquito. Effective coverage represents the proportion of people who are actually protected from ongoing transmission, and is always less than coverage.
...
itn_data <- data.frame(
  ...
  pivot_longer(!Year, names_to = "Type", values_to = "Percent")

# Create the ITN coverage plot
ggplot(itn_data, aes(x = Year)) +
  geom_line(aes(y = Percent, colour = Type)) +
  theme_health_radar() +
...
Mosquito nets are intended to prevent transmission by reducing the contact rate between humans and mosquitoes, and we assume that the population that is effectively covered by a mosquito net is no longer vulnerable to infection. As such, the force of infection now excludes those protected by ITNs.
:::
<<<<<<< HEAD:datasources/malaria/dhs-program/dhs.qmd

:::
=======
>>>>>>> main:radar/datasources/malaria/dhs-program/dhs.qmd
