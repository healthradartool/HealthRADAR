...
:::
:::{.cell}
![Daily rainfall time series for Mwinilunga, Zambia, from 1959 to 2023, derived from ERA5 data.
...
ERA5 data is available at hourly resolution and can be aggregated to daily or monthly time scales. The dataset includes many surface and atmospheric variables and is updated daily with only a few days' delay, making it suitable for near-real-time analysis. As a model-observation hybrid, ERA5 offers spatially and temporally complete outputs, which can be particularly valuable in regions or periods with limited direct observations.
