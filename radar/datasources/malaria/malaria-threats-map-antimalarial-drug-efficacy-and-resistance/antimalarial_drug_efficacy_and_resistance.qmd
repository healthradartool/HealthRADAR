...
mean_pf <- therapeutic_eff_data |> 
  ...
  mutate(time = c(365, 365*5, 365*8)) # to get time series for 2010, 2014 and 2017
...
df <- as_tibble(as.data.frame(out)) |> 
  ...
  mutate(date = ymd("2009-01-01") + time)

df |> 
  filter(variable %in% "Inc", date > ymd("2009-06-01")) |> 
  ggplot() +
  ...
  labs(
    ...
    x = "Year",
    y = "Incidence",
    caption = str_wrap("Incidence follows the seasonal pattern of malaria, but has an overall decreasing trend as treatment failure decreases over time, and more infected individuals are successfully cured. Source: Model output")
  )  +
...
for (i in 1:runs) {
  ...
  pf_data <- pf_data |>  
    mutate(
    pf_sampled = pmap_dbl(list(min, mode, max), ~ rtri(1, min = ..1, mode = ..2, max = ..3)),
    time = c(365, 365*5, 365*8) # to get time series for 2010, 2014 and 2017
  )

  # Time-dependent interpolation for sampled values
 pf_run_function <- approxfun(pf_data$time, pf_data$pf_sampled, rule = 2)  # linear interpolation
...
median_sens <- sens_df |> 
  group_by(time) |> 
  filter(variable == "Inc") |> 
  summarise(median_Inc = median(value), 
            lower_Inc = quantile(value, 0.05),
            upper_Inc = quantile(value, 0.95),
            .groups = "drop") |> # plot median values and 95% confidence intervals
  mutate(date = ymd("2009-01-01") + time)

sens_df |> 
  filter(variable == "Inc", date > ymd("2009-06-01")) |> 
  ggplot() +
  #geom_line(aes(x = date, y = value, colour = as_factor(run), group = run), alpha = 0.5) + # if you want to see individual runs
  geom_ribbon(data = median_sens |> filter(date > ymd("2009-06-01")), aes(x = date, ymin = lower_Inc, ymax = upper_Inc), 
              fill = theme_health_radar_colours[12], alpha = 0.5) +
  geom_line(data = median_sens |> filter(date > ymd("2009-06-01")), aes(x = date, y = median_Inc, linetype = "Median"), linewidth = 0.5) +
  theme_health_radar() +
  scale_y_continuous(labels = scales::label_number(suffix = " K", scale = 1e-3)) +
  labs(
    title = "Median country-wide daily incidence in Zimbabwe", 
    subtitle = str_wrap("Shaded area shows 5–95% range across 50 model runs."),
    caption = str_wrap("Sensitivity analysis of values for treatment failure in each year. Sampling from within the minimum and maximum ranges provided does not appear to significantly cause changes to incidence. Source: Model output."),
    x = "Year",
    y = "Incidence",
    linetype = "") +
  guides(colour = "none") 
