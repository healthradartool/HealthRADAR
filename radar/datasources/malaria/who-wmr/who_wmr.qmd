...
seacr <- function(times, start, parameters, itnfunc) { 
  with(as.list(c(start, parameters)), {
    ...
    phi = 93 # phase angle; start of season
...
initial_state <- c(Sm = 20000000, # susceptible mosquitoes
                   ...
                   ITN = 0.5 # # coverage of the population with potential to be protected by nets CURRENTLY IN CIRCULATION
)
...
initial_parameters <- c(a = 0.3, # human biting rate
                ...
                zeta_a = 0.28, # relative infectiousness of asymptomatic infections
...
nll_seacrs <- function(initial_parameters, observed_data) {
  ...
  annual_incidence_rate  <- model_output |> 
    ...
    filter(year > 2012, year < 2018) # select years between 2013 and 2018 for fitting
