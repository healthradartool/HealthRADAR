...
This allows the model to capture the fact that not all malaria cases are treated due to the asymptomatic manifestation of the disease—particularly in high-burden, endemic contexts—as well as human behavioural factors and limitations within the health system.

### Simulated calibration

Following model calibration and fine-tuning of the parameters, we anticipate model outputs that mimic the pattern and overall trajectory of the estimated data. We simulate an example below:
```{r}

# Simulated model output with 95% Confidence intervals of incidence rates
modelled_incidence_rate <- data.frame(
  Year = 2000:2022,
  incidence_rate = estimated_data$inci_rate + rnorm(length(estimated_data$inci_rate), mean = 50, sd = 25)) |>  
  mutate(
    std_error = 0.03, # for example
