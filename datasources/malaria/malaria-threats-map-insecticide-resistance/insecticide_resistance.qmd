...
<h2>Key points to consider</h2>
  ...
  - The data include assays conducted with fewer than the minimum recommended number of mosquitoes (i.e. 100 female *anopheline* mosquitoes tested in **WHO susceptibility tests**^[A standardized bioassay to assess mosquito mortality after exposure to a diagnostic dose of insecticide, used to monitor resistance]) as in the absence of other data these results are informative despite the small sample size.
...
Resistance to **Pyrethroids**^[A class of synthetic insecticides widely used in malaria control (e.g., in bed nets and sprays) that target mosquito nervous systems] has been identified from most countries in Africa but there is a scarcity of data on insecticide resistance intensity. The above figure shows the distribution of the intensity of pyrethroid resistance on the African continent which ranges from low to moderate in the east and south to moderate to high in the west. This map is made up of 1445 bioassays with the majority being in north-western Africa.
...
insecticide_data <- read_excel("data/MTM_DISCRIMINATING_CONCENTRATION_BIOASSAY_20250330.xlsx", sheet = 2)
