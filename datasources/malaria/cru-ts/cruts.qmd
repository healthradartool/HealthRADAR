...
- The **skill**^[In climate science, the *skill* of a dataset refers to its accuracy, resolution, bias, uncertainty, and predictive power in representing real-world climate conditions. A high-skill dataset closely matches observations, has minimal bias, and reliably supports climate modelling and decision-making] of the dataset relies heavily on the density of the station network. The density over southern Africa is very poor with some areas not having any available stations, or they are are very far away in climatically very different regions.
...
# Load the data 
temperature_values <- read_csv("data/Fig5_tas_mon_CRU_TS-4.08_190101-202312_Bulawayo.csv") |> 
  select(time, tas) |> 
