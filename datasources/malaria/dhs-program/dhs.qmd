...
subtitle: "The Demographic and Health Surveys (DHS) Program has collected population survey data from over 90 countries for over 30 years."
...
{{< video https://www.youtube.com/watch?v=eXdPi6a_pQ0&list=PLagqLv-gqpTNRdiY391wMY0lzM_MuYpAL >}}

**`rdhs` R package**

[`rdhs`](https://docs.ropensci.org/rdhs/) is a package for management and analysis of Demographic and Health Survey (DHS) data. This includes functionality to:
...
**Accessing the data using STATcompiler abilities**
...
-   Operational effectiveness accounts for factors such as the peak biting hours of a particular vector versus the time of day actually spent under the net. \*\*add other text here
...
To demonstrate a loss in net functionality and effectiveness as they degrade and undergo attrition, the user can use the concept of convolution to represent a reduction in net access in the population. The initial series of values (net access) is reduced by another series of values (itn effectiveness) at each time step by "filtering" the initial series through the reducing series. For example,
...
So 95% of nets received in year one will remain. In year two, 95% of nets received in year two will remain, as well as 90% of the nets previously received in year one. In year three, 95% of any new nets will remain, 90% of the nets received in year two, and 85% of the nets previously received in year one. After four years, 0% of the nets received in year one remain.
...
The measure of intensity of disease transmission is also known as the force of infection. Simply put, this is the rate at which suspectible individuals become infected; as is affected by the proportion of the mosquito population that is infectious, the contact rate between susceptible humans and infectious mosquitoes and the probability that a bite from an infectious mosquito transmits disease to a susceptible human. Mosquito nets are intended to prevent transmission by reducing the contact rate between humans and mosquitoes, and we assume that the population that is effectively covered by a mosquito net is no longer vulnerable to disease. As such, the force of infection, typically denoted by lambda ($λ$), now excludes those protected by the mosquito nets.
