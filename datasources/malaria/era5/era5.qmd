...
Mosquito survival and malaria transmission are strongly influenced by short-term changes in temperature, humidity, and rainfall. Datasets like ERA5, with high temporal resolution, allow us to identify when critical climatic thresholds are crossed. 
In this example, we show how ERA5 data can be used to assess the ‘climatic suitability’ for malaria transmission by visualising key thresholds of rainfall, temperature, and humidity that influence mosquito survival and parasite development.

To generate the visualisations, we extracted ERA5 reanalysis hourly temperature (2m temperature), dewpoint temperature (2m dewpoint temperature), and precipitation (‘Total precipitation’) data. Time-series data was extracted for these three variables for the period 2010-2020 for the two grid cells with in which Kariba (Zimbabwe) and Mbabane (Eswatini) are found. The following preparation steps were applied:

- Rainfall: Hourly precipitation was first summed to daily totals. A 7-day rolling sum (i.e. the total rainfall for today and previous six days) was then applied, to capture short-term wet periods that are more relevant to mosquito breeding than isolated rain events.

- Temperature:  Hourly temperature values were averaged to produce daily mean temperature.

- Relative humidity: Relative humidity is not directly available in the ERA5 dataset. Instead, it was derived from daily mean temperature ($T$) and dewpoint temperature ($T_d$) using the August–Roche–Magnus formulas for saturation vapor pressure ($e_s$), actual vapor pressure ($e$), and relative humidity ($RH$):


$$
e_s = 6.112 \times \exp\left(\frac{17.67 \, T}{T + 243.5}\right)
$$

$$
e = 6.112 \times \exp\left(\frac{17.67 \, T_d}{T_d + 243.5}\right)
$$
...
To assess climatic suitability for malaria transmission, we analyze ERA5 data through three key visualizations (Fig 1–3), focusing on three climatic thresholds critical to Anopheles mosquitoes and Plasmodium parasites:

1. Daily Temperature (18–32°C): Bounds for parasite development.
2. Daily relative humidity (>60%): Minimum for mosquito survival.
3. 7-day rolling sum of daily precipitation (>10mm): Triggers breeding-site formation.

When all three thresholds are met simultaneously, conditions become optimal for mosquito survival and parasite development. Figures 1 (daily thresholds), 2 (seasonal patterns), and 3 (annual trends) highlight when and where these conditions occur. These insights could help target interventions when and where climate-driven risk is highest.

:::{.cell}
![Fig. 1: Climatic thresholds of daily average temperature and relative humidity and of a 7-day rolling total precipitation over Kariba and Mbabane from 2010-2020](images/Vis1A_Thresholds-ERA5_2010-2020.png)
:::

Figure 1 visualises the periods when climatic thresholds are met for both Kariba (Zimbabwe) and Mbabane (Eswatini). The top panel shows daily temperature with thresholds of 18 °C and 32 °C, approximating the lower and upper bounds for mosquito survival and parasite development. The middle panel shows daily relative humidity, with a threshold of 60%. Sustained humidity above this level supports mosquito survival and activity, while lower humidity limits their ability to thrive. The bottom panel shows the 7-day rolling sum of daily precipitation. Periods where rainfall exceeds 10 mm/7 days are highlighted, representing conditions favourable for mosquito breeding.


:::{.cell}
![Fig. 2: Average percentage of climatically suitable days per month over Kariba and Mbabane from 2010-2020](images/Vis3_Monthly-Climatic-Suitability_ERA5_2010-2020.png)
:::

Figure 2 condenses the information shown in Figure 1 into a heatmap illustrating the average percentage of days in each calendar month when the climatic thresholds for malaria transmission are met for both Kariba (Zimbabwe) and Mbabane (Eswatini). Clear seasonality is evident for both locations. The warm, humid, and wet summer months from December to February exhibit the highest proportion of suitable days (>70%), providing favorable conditions for mosquito survival and malaria transmission. In contrast, although Kariba remains warm enough during winter, both Kariba and Mbabane experience very few suitable days in the cooler, drier, and less humid months, as the climatic thresholds are rarely met.

:::{.cell}
![Fig. 3: Total number of climatically suitable days per year over Kariba and Mbabane from 2010-2020](images/Vis2_Annual-Climatic-Suitability_ERA5_2010-2020.png)
:::

Figure 3 shows the total number of climatically suitable days—when all three climatic thresholds are met—for each year from 2010 to 2020 in both Kariba and Mbabane. Despite being in different locations, both sites experience a similar number of climatically suitable days each year. Over the 2010–2020 period, Kariba experienced an average of 118 climatically suitable days per year, while Mbabane experienced an average of 126 days. There is a bit of inter-annual variability with 2019 showing experience the lowest number of suitable days in Kariba (76) and the highest in Mbabane (149). 

In this example, we demonstrated how ERA5’s high temporal resolution can track climatic thresholds critical for malaria transmission in Kariba and Mbabane. By aggregating hourly data to daily/weekly scales, we identified periods of temperature, humidity, and rainfall suitability for Anopheles mosquitoes and Plasmodium parasites. However, three key limitations arise:

1. Temporal smoothing: Daily averages obscure sub-daily temperature extremes that may impact mosquito survival.
2. Rainfall granularity: The 7-day rolling sum may overlook brief, intense storms that both create transient breeding sites and flush away existing ones—a dual effect critical for malaria risk.
3. Threshold simplicity: Fixed thresholds cannot capture species-specific tolerances or nonlinear interactions between variables.

Despite these constraints, this example provides a replicable framework to assess climate-driven malaria risk—useful for prioritizing surveillance or interventions when thresholds are crossed.
