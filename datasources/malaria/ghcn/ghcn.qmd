...
ggplot() +
  ...
  labs(
    ...
    caption = str_wrap("Limpopo’s higher summer temperatures significantly accelerate the parasite's Extrinsic Incubation Period (EIP), creating (high-velocity) transmission windows. Maputo’s stable thermal profile suggests a consistent, year-round window for vector competence. Source: GHCN Daily (TAVG)",
      width = 150)
    )+
  ylim(0, NA)
...
df |> 
  ...
  labs(
    ...
    y = "Population size",
    colour = "Population",
    caption = str_wrap("In Limpopo, mosquito density (M) follows an opportunistic (boom-and-bust) cycle in response to rainfall-driven habitat availability. Maputo’s sustained population levels, particularly between 2020 and 2022, suggest favorable environmental conditions that prevent the population from crashing during the dry season, ensuring year-round biting pressure. Source: SEACR-SEI Model.", width = 150)
  )
...
This is especially insightful for comparing epidemic versus endemic transmission dynamics. Note that the outputs below are illustrative models of transmission potential based on environmental suitability. In reality, the number of reported clinical cases may differ due to human interventions, limitations in treatment access, availability of rapid diagnostic tests, and test sensitivity.
```{r}
df |> 
...
Models that explicitly incorporate localised temperature and rainfall are critical for shifting public health strategies. By understanding the distinct epidemiological profiles of regions such as Limpopo and Maputo, policymakers can optimise resource allocation:
...
In areas where environmental conditions support year-round vector competence, policy must focus on sustained, continuous interventions. This includes ensuring universal coverage of Long-Lasting Insecticidal Nets (LLINs), robust seasonal malaria chemoprevention (SMC), and continuous supply chains for rapid diagnostic tests (RDTs) and antimalarials.
