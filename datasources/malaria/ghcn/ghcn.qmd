...
GHCNd is a composite dataset, integrating daily climate observations from approximately 30 different data sources that have been merged into a single consistent format. GHCNd undergoes rigorous quality assurance reviews and data is updated daily with a latency of 24-48 hours for many stations. Each daily update to GHCNd is assigned a unique version number, and then archived at NCEI.
...
For a comprehensive list of variables, you can refer to the dataset's documentation on the [GHCNd Documentation](https://www.ncei.noaa.gov/data/global-historical-climatology-network-daily/doc/GHCND_documentation.pdf).
...
- [Documentation](https://www.ncei.noaa.gov/data/global-historical-climatology-network-daily/doc/GHCND_documentation.pdf): Data format, element definitions, station variables, and quality and source flag definitions.
...
Below are a few plots to give a better sense of where the GHCNd stations are located. 
...
:::{.cell}
![Spatial map of GHCNd Average Temperature stations over southern Africa. The size of the station marker indicates the length of the station and the colour indicates the last year the station produced data.](images/ghcnd_TAVG_stations_size.png){fig-align="center"}
:::
...
The GHCNd dataset consists of direct station observations rather than model outputs, presenting both unique advantages and analytical challenges. Unlike spatially complete gridded datasets, GHCNd exhibits uneven geographic coverage with particularly sparse station density across much of Africa outside urban centers and former colonial observation networks, while stations in temperate regions often benefit from longer and more complete records.

Approximately half of all GHCNd stations report precipitation only. Users requiring temperature data should be aware that the network is considerably sparser for temperature variables, and should check station availability for their region of interest before selecting GHCNd for their analysis.

Individual station histories frequently contain gaps due to operational interruptions, and many records are discontinuous as stations relocate or change instrumentation. This transition may introduce artificial shifts in the data unless properly accounted for through metadata analysis.

While NOAA applies rigorous quality control including outlier detection and homogenization algorithms to create climate-quality data, residual errors may persist at individual stations due to localized effects like urbanization or microclimate influences that aren't fully corrected in the automated processing.

These characteristics make GHCNd exceptionally valuable for validating gridded products and studying local climate extremes, but require careful preprocessing including gap-filling, homogeneity testing, and urban heat island screening when used for trend analysis or combined into regional composites. The dataset's strength lies in its direct measurements of weather conditions, but this comes with the responsibility for users to thoroughly understand each station's history and potential artifacts before drawing climate conclusions.

Although the GHCNd dataset is updated daily, many African stations are updated irregularly and may rely on Global Summary of the Day (GSOD) feeds, with some historical records not extending to recent years, leading to delayed or incomplete data coverage compared with other regions.

As GHCNd provides point measurements from individual stations, there is no perfect way to compare these directly with gridded model or reanalysis output. Techniques such as spatial interpolation or nearest-neighbour grid cell extraction are commonly used, but users should be aware that differences may reflect the spatial scale mismatch rather than true disagreement between datasets.
...
Menne, M.J., I. Durre, R.S. Vose, B.E. Gleason, and T.G. Houston, 2012: An overview of the Global Historical Climatology Network-Daily Database. Journal of Atmospheric and Oceanic Technology, 29, 897-910. [DOI:10.1175/JTECH-D-11-00103.1](https://doi.org/10.1175/JTECH-D-11-00103.1)

<h3>Terms of use</h3>

GHCNd data are produced by NOAA and are freely available for public use. As a U.S. federal government dataset, the data are in the public domain in the United States, and NCEI works to ensure a Creative Commons CC0 license is applied. NOAA requests attribution for the use or dissemination of unaltered data, and it is not permissible to state or imply endorsement by or affiliation with NOAA. For further details, refer to the [NESDIS/NCEI Open Data Policy](https://www.ncei.noaa.gov/archive#v-pills-licensing).
...
For the ERA5 data, we used daily average, minimum, and maximum temperature, as well as precipitation. Daily NetCDF files for each variable were concatenated along the time dimension and the time series for the grid cell nearest to Skukuza (latitude -24.983°, longitude 31.6°) was extracted using nearest-neighbour selection.
...
:::{.cell}
![Fig. 1: Daily rainfall time series for Skukuza, South Africa, from 1911 to 2025. Lighter shaded bars indicate values in the GHCN station record that are sourced from the Global Summary of the Day (GSOD).](images/Vis1_GHCN_daily_TimeSeries_Rainfall_Skukuza.png)
:::
...
:::{.cell}
![Fig. 3: Annual rainfall comparison between the GHCN station record and the ERA5 grid cell over Skukuza, South Africa. The black dashed line indicates the percentage of valid daily rainfall observations available in the GHCN record for each year.](images/Vis3_GHCN-ERA5_year_TimeSeries_Rainfall_Skukuza.png)
:::
...
:::{.cell}
![Fig. 5: Comparison of monthly climatologies of rainfall between the GHCN station record (blue bars) and the ERA5 grid cell (orange bars) from 1960 to 2000 over Skukuza, South Africa. The dashed lines show the average number of rain days (≥1 mm) for the GHCN record (dark blue) and ERA5 (dark orange).](images/Vis5_GHCN-ERA5_monthly_Climatology_Rainfall_Skukuza.png)
:::
...
:::{.cell}
![Fig. 6: Comparison of monthly climatologies of maximum (red) and minimum (yellow) temperatures between the GHCN station record (solid line) and the ERA5 grid cell (dashed line) from 1960 to 2000 over Skukuza, South Africa.](images/Vis6_GHCN-ERA5_monthly_Climatology_Temperatures_Skukuza.png)
:::
