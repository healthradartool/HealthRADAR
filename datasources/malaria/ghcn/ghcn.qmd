...
::: {.panel-tabset}
### Data Creator
The  is the creator and maintainer of the GHCNd dataset.

The Global Historical Climatology Network – Daily (GHCNd) dataset is created and maintained by the [National Centers for Environmental Information (NCEI)](https://www.ncei.noaa.gov/). Users can download it directly from the GHCNd's data directory [page](https://www.ncei.noaa.gov/data/global-historical-climatology-network-daily/), which provides access to the latest version along with supporting documentation. Daily data from GHCN are available as individual ASCII (i.e., plain text) files (one file per station) and as a GZIP-compressed TAR file containing all of the station files. The descriptions of the data format of available metadata and data files are all laid out in the GHCNd [readme file](). This is the best source for accessing the most up-to-date version of the dataset.
