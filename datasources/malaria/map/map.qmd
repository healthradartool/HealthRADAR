...
data <- read.csv("data/Subnational_Unit-data.csv")
...
shp_zambia <- sf::st_read(
            "data/Zambia_-_Administrative_District_Boundaries_2022/Zambia_-_Administrative_District_Boundaries_2022.shp", 
  quiet = TRUE) |>  
  filter(PROVINCE %in% c("Eastern", "Northern", "Southern", "Western")) 


# Load raster file containing walking only travel time to healthcare
raster_walk <- rast("data/Global_Walking_Only_Travel_Time_To_Healthcare_ZMB.tiff")
...
IRSdata <- read.csv("data/National_Unit-data.csv") |>  
